...
import { BoundaryCaptureModal } from '../components/BoundaryCaptureModal'; // 🚀 Import Modal

// 🚀 Make sure you added handleBoundaryCapture to the destructured props
export const Step5MediaAndDigital = ({ form, t, handleCameraUpload, handleBoundaryCapture, uploading }: any) => {
  ...
  const renderPhotoTask = (key: string, label: string, desc: string, allowVideo: boolean = false) => (
    <View style={{ backgroundColor: '#F8FAFC', padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
      ...
      {docs[key] ? (
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#DCFCE7', padding: 12, borderRadius: radius.md }}>
        ...
        // 🚀 Pass allowVideo to the handler and dynamically update icon/text
  ...
  return (
    <View>
      ...
      {/* Task 1 now triggers the Walk mode */}
      ...
      {/* 🚀 Mount the Boundary Tracker Modal */}
