...
const AdditionalShopLocation = ({ form, index }: { form: UseFormReturn<DealerOnboardingValues>, index: number }) => {
  ...
  return (
    <>
      <Controller control={control} name={`additionalShops.${index}.state`} render={({field}) => <SelectField label={t("State *")} value={field.value ?? ''} options={INDIAN_STATES} searchable onChange={(val) => { field.onChange(val); setValue(`additionalShops.${index}.city`, ''); setValue(`additionalShops.${index}.taluka`, ''); setValue(`additionalShops.${index}.village`, ''); }} error={form.formState.errors.additionalShops?.[index]?.state?.message} />} />
      <Controller control={control} name={`additionalShops.${index}.city`} render={({field}) => <SelectField label={loadingLoc ? t("City/District (Loading...) *") : t("City/District *")} value={field.value ?? ''} options={cities} searchable onChange={(val) => { field.onChange(val); setValue(`additionalShops.${index}.taluka`, ''); setValue(`additionalShops.${index}.village`, ''); }} error={form.formState.errors.additionalShops?.[index]?.city?.message} />} />
      <Controller control={control} name={`additionalShops.${index}.taluka`} render={({field}) => <SelectField label={t("Taluka/Tehsil *")} value={field.value ?? ''} options={talukas} searchable onChange={(val) => { field.onChange(val); setValue(`additionalShops.${index}.village`, ''); }} error={form.formState.errors.additionalShops?.[index]?.taluka?.message} />} />
      <Controller control={control} name={`additionalShops.${index}.village`} render={({field}) => <SelectField label={t("Village *")} value={field.value ?? ''} options={villages} searchable onChange={field.onChange} error={form.formState.errors.additionalShops?.[index]?.village?.message} />} />
    </>
  );
};
    ...
    <View>
      ...
      {watch('hasAdditionalLocations') === 'Yes' && (
        <View style={{ marginBottom: spacing.xl, padding: spacing.md, backgroundColor: '#F8FAFC', borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border }}>
          ...
          <Text style={{ fontWeight: '800', color: colors.textMuted, marginBottom: spacing.sm }}>{t("Additional Shops")}</Text>
          {additionalShops.map((shop, i) => (
            <View key={i} style={{ backgroundColor: '#FFF', padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.sm }}>
                <Text style={{ fontWeight: '700', color: colors.text }}>{t("Additional Shop")} {i + 1}</Text>
              ...
              <Controller control={control} name={`additionalShops.${i}.shopName`} render={({field}) => <Input label={t("Shop Name *")} value={field.value} onChangeText={field.onChange} placeholder={t("e.g. Kisan Seva Kendra")} error={form.formState.errors.additionalShops?.[i]?.shopName?.message} />} />
              <Controller control={control} name={`additionalShops.${i}.estYear`} render={({field}) => <YearPickerField label={t("Establishment Year *")} value={field.value ?? ''} onChange={field.onChange} placeholder={t("Select Year")} error={form.formState.errors.additionalShops?.[i]?.estYear?.message} />} />
              
              {/* Uses the sub-component for independent cascading location logic */}
              <AdditionalShopLocation form={form} index={i} />
              
              <Controller control={control} name={`additionalShops.${i}.address`} render={({field}) => <TextArea label={t("Full Address *")} value={field.value} onChangeText={field.onChange} error={form.formState.errors.additionalShops?.[i]?.address?.message}/>} />
            </View>
          ))}
          <Pressable onPress={() => setValue('additionalShops', [...additionalShops, { shopName: '', estYear: '', state: '', city: '', taluka: '', village: '', address: '' }])} style={{ padding: 12, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: radius.md, alignItems: 'center', marginBottom: spacing.lg }}>
            <Text style={{ color: colors.primary, fontWeight: '800' }}>{t("+ Add Another Shop")}</Text>
          </Pressable>

          <Text style={{ fontWeight: '800', color: colors.textMuted, marginBottom: spacing.sm }}>{t("Godowns / Storage")}</Text>
          {godowns.map((godown, i) => (
            <View key={i} style={{ backgroundColor: '#FFF', padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.sm }}>
                <Text style={{ fontWeight: '700', color: colors.text }}>{t("Godown")} {i + 1}</Text>
              ...
              <Controller control={control} name={`godowns.${i}.address`} render={({field}) => <TextArea label={t("Full Address *")} value={field.value} onChangeText={field.onChange} error={form.formState.errors.godowns?.[i]?.address?.message}/>} />
              <View style={{ flexDirection: 'row', gap: spacing.sm }}>
                <View style={{ flex: 2 }}><Controller control={control} name={`godowns.${i}.capacity`} render={({field}) => <Input label={t("Capacity *")} value={field.value} onChangeText={field.onChange} keyboardType="numeric" placeholder={t("e.g. 500")} error={form.formState.errors.godowns?.[i]?.capacity?.message} />} /></View>
                <View style={{ flex: 1 }}><Controller control={control} name={`godowns.${i}.capacityUnit`} render={({field}) => <SelectField label={t("Unit *")} value={field.value || 'Sq.ft'} options={['Sq.ft', 'Sq.m']} onChange={field.onChange} error={form.formState.errors.godowns?.[i]?.capacityUnit?.message} />} /></View>
              </View>
            </View>
          ))}
          <Pressable onPress={() => setValue('godowns', [...godowns, { address: '', capacity: '', capacityUnit: 'Sq.ft' }])} style={{ padding: 12, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: radius.md, alignItems: 'center' }}>
            <Text style={{ color: colors.primary, fontWeight: '800' }}>{t("+ Add Godown")}</Text>
          </Pressable>
        </View>
      )}

      <Controller control={control} name="isLinkedToDistributor" render={({field}) => <RadioGroup label={t("Are you linked to any distributor? *")} options={['Yes', 'No']} value={field.value} onChange={(val) => { field.onChange(val); if (val === 'No') setValue('linkedDistributors', []); else setValue('linkedDistributors', [{name: '', contact: ''}]); }} />} />
      {watch('isLinkedToDistributor') === 'Yes' && (
        <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
          <Input label={t("Distributor Name *")} placeholder={t("e.g. Ramesh Agro")} value={watch('linkedDistributors')?.[0]?.name || ''} onChangeText={(val) => { const current = watch('linkedDistributors')?.[0] || { name: '', contact: '' }; setValue('linkedDistributors', [{ name: val, contact: current.contact }], { shouldValidate: true }); }} />
          <Input label={t("Contact *")} placeholder="9876543210" value={watch('linkedDistributors')?.[0]?.contact || ''} onChangeText={(val) => { const current = watch('linkedDistributors')?.[0] || { name: '', contact: '' }; setValue('linkedDistributors', [{ name: current.name, contact: val }], { shouldValidate: true }); }} keyboardType="phone-pad" maxLength={10} prefix="+91" />
        </View>
      )}

      <Controller control={control} name="proposedStatus" render={({field}) => <RadioGroup label={t("Proposed Status *")} options={['Authorised Dealer', 'Exclusive Dealer', 'Dealer']} value={field.value} onChange={field.onChange} error={form.formState.errors.proposedStatus?.message} />} />
      
      <Controller control={control} name="willingDemoFarmers" render={({field}) => <RadioGroup label={t("Willing to work with 5–10 demo farmers per season? *")} options={['Yes', 'No']} value={field.value} onChange={field.onChange} />} />
      
      {watch('willingDemoFarmers') === 'Yes' && (
        <View style={{ marginTop: spacing.md, marginBottom: spacing.xl }}>
          <Text style={{ fontWeight: '800', color: colors.text, marginBottom: spacing.sm }}>{t("Demo Farmers List")}</Text>
          <Text style={{ fontSize: 12, color: colors.textMuted, marginBottom: spacing.md }}>{t("Fill out details manually, or take a picture of their list.")}</Text>
          ...
          {!watch('documents')?.['demo_farmers_list'] && (
            <View style={{ marginTop: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, overflow: 'hidden' }}>
              <View style={{ flexDirection: 'row', backgroundColor: colors.primarySoft, padding: 8, borderBottomWidth: 1, borderBottomColor: colors.border }}>
                <Text style={{ flex: 1.5, fontWeight: '700', fontSize: 12, color: colors.primary, textAlign: 'center' }}>{t("Name")}</Text>
                <Text style={{ flex: 1.5, fontWeight: '700', fontSize: 12, color: colors.primary, textAlign: 'center' }}>{t("Contact")}</Text>
                <Text style={{ flex: 2, fontWeight: '700', fontSize: 12, color: colors.primary, textAlign: 'center' }}>{t("Address")}</Text>
              </View>
          ...
          {!watch('documents')?.['demo_farmers_list'] && (
            <Pressable onPress={() => setValue('demoFarmers', [...demoFarmers, { name: '', contact: '', address: '' }])} style={{ marginTop: spacing.sm, alignSelf: 'flex-start' }}>
              <Text style={{ color: colors.primary, fontWeight: '700', fontSize: 13 }}>{t("+ Add Row")}</Text>
            </Pressable>
          )}
        </View>
      )}
