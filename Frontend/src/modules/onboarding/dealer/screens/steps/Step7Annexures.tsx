...
export const Step7Annexures = ({ 
  ...
  return (
    <View>
      ...
      <Text style={{ color: colors.textMuted, marginBottom: spacing.lg }}>{t("Fill out the territory and business details to generate the final agreement.")}</Text>
      ...
      <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
          ...
          return (
            <View key={index} style={{ backgroundColor: '#F8FAFC', padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.sm }}>
                <Text style={{ fontWeight: '700', color: colors.text }}>{t("Territory")} {index + 1}</Text>
                {index > 0 && (
              ...
              <Controller control={control} name={`seTerritories.${index}.taluka`} render={({field}) => (
                <SelectField label={t("Taluka *")} value={field.value ?? ''} options={talukas.length > 0 ? talukas : [t('Select City in Step 1 first')]} searchable onChange={(val) => { field.onChange(val); setValue(`seTerritories.${index}.village`, []); }} error={form.formState.errors.seTerritories?.[index]?.taluka?.message} />
              )} />
              
              <Controller control={control} name={`seTerritories.${index}.village`} render={({field}) => (
                <MultiSelectField label={t("Village(s) *")} value={field.value ?? []} options={rowVillages.length > 0 ? rowVillages : [t('Select Taluka first')]} searchable onChange={field.onChange} error={form.formState.errors.seTerritories?.[index]?.village?.message} />
              )} />

              <Controller control={control} name={`seTerritories.${index}.cultivableArea`} render={({field}) => <Input label={t("Cultivable Area (in Acres) *")}   value={field.value} onChangeText={field.onChange} keyboardType="numeric" placeholder="e.g. 1500" error={form.formState.errors.seTerritories?.[index]?.cultivableArea?.message} />} />
              <Controller control={control} name={`seTerritories.${index}.majorCrops`} render={({field}) => <MultiSelectField label={t("Major crops in territory *")} value={field.value || []} options={WEST_INDIA_CROPS} onChange={field.onChange} searchable placeholder={t("Select crops")} error={form.formState.errors.seTerritories?.[index]?.majorCrops?.message} />} />
            </View>
          );
        })}
        ...
        <Pressable onPress={() => setValue('seTerritories', [...territories, { taluka: '', village: [], cultivableArea: '', majorCrops: [] }])} style={{ padding: 12, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: radius.md, alignItems: 'center' }}>
          <Text style={{ color: colors.primary, fontWeight: '800' }}>+ {t("Add Another Territory")}</Text>
        </Pressable>
      </View>

      <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
        <Text style={{ fontWeight: '800', fontSize: 16, marginBottom: spacing.sm, color: colors.primary }}>{t("Annexure B: Principal Companies & Products")}</Text>
        <Controller control={control} name="sePrincipalSuppliers" render={({field}) => <MultiSelectField label={t("List of current principal suppliers *")} value={field.value || []} options={DEMO_SUPPLIERS} onChange={field.onChange} searchable placeholder={t("Select suppliers")} />} />
        <Controller control={control} name="seChemicalProducts" render={({field}) => <MultiSelectField label={t("Chemical products range *")} value={field.value || []} options={DEMO_CHEMICALS} onChange={field.onChange} searchable placeholder={t("Select chemicals")} />} />
        <Controller control={control} name="seBioProducts" render={({field}) => <MultiSelectField label={t("Biological / organic products range *")} value={field.value || []} options={DEMO_BIOS} onChange={field.onChange} searchable placeholder={t("Select biologicals")} />} />
        <Controller control={control} name="seOtherProducts" render={({field}) => <MultiSelectField label={t("Other products (seeds, etc.) *")} value={field.value || []} options={DEMO_OTHERS} onChange={field.onChange} searchable placeholder={t("Select other products")} />} />
      </View>

      <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
        <Text style={{ fontWeight: '800', fontSize: 16, marginBottom: spacing.sm, color: colors.primary }}>{t("Annexure D: Bank & Credit References")}</Text>
        <Controller control={control} name="seHasCreditReferences" render={({field}) => (
          <RadioGroup label={t("Add supplier references? ")} options={['Yes', 'No']} value={field.value} onChange={(val) => { field.onChange(val); if (val === 'No') setValue('seCreditReferences', []); else setValue('seCreditReferences', [{name: '', contact: '', behavior: '', behaviorAudio: ''}]); }} />
        )} />
          ...
          <View style={{ marginTop: spacing.sm }}>
            {watch('seCreditReferences')?.map((ref, index) => (
              <View key={index} style={{ padding: spacing.sm, backgroundColor: '#F8FAFC', borderRadius: radius.md, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.border }}>
                ...
                <Input label={t("Supplier Name *")} value={ref.name} placeholder="e.g. Ramesh Agro" onChangeText={(val) => { const refs = [...(watch('seCreditReferences') || [])]; refs[index].name = val; setValue('seCreditReferences', refs, {shouldValidate: true}); }} />
                <Input label={t("Contact Number *")} value={ref.contact} placeholder="9876543210" prefix="+91" maxLength={10} keyboardType="phone-pad" onChangeText={(val) => { const refs = [...(watch('seCreditReferences') || [])]; refs[index].contact = val; setValue('seCreditReferences', refs, {shouldValidate: true}); }} />
                <Text style={{ fontWeight: '700', fontSize: 12, marginBottom: 4, marginTop: 8 }}>{t("Last 12-month payment behavior")} </Text>
                <TextArea label={t("Behavior Notes")} placeholder="Type behavior here..." value={ref.behavior} onChangeText={(val) => { const refs = [...(watch('seCreditReferences') || [])]; refs[index].behavior = val; setValue('seCreditReferences', refs, {shouldValidate: true}); }} />
      ...
      <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
        <Text style={{ fontWeight: '800', fontSize: 16, marginBottom: spacing.sm, color: colors.primary }}>{t("Annexure E & F: Sales & Expansion")}</Text>
        ...
        <View style={{ marginTop: spacing.md }}>
          ...
          <Controller control={control} name="seGrowthVision" render={({field}) => <TextArea label={t("Vision Notes")} placeholder={t("Type vision here...")} value={field.value} onChangeText={field.onChange} />} />
      ...
      {watch('seSecurityDeposit') && parseInt(watch('seSecurityDeposit') || '0') > 0 && (
        <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }}>
          <Text style={{ fontWeight: '800', fontSize: 16, marginBottom: spacing.xs, color: colors.primary }}>{t("Payment Proof *")}</Text>
          ...
          <Controller 
            ...
            render={({field}) => (
              <Input 
                label={t("Transaction ID or Cheque Number")} 
                value={field.value} 
                onChangeText={field.onChange} 
                placeholder={t("e.g. TXN123456789 or CHQ000123")} 
