...
export const Step6Documents = ({ form, uploading, handleUpload }: Props) => {
  ...
  return (
    <View>
      ...
      <Text style={{ fontWeight: '700', color: colors.textMuted, marginBottom: spacing.sm }}>{t("Core Documents")}</Text>
      ...
      <Text style={{ fontWeight: '700', color: colors.textMuted, marginBottom: spacing.sm, marginTop: spacing.lg }}>{t("Shop Photos (Capture Multiple)")}</Text>
      {[
        { key: 'shop_exterior', label: t('1. Exterior (Store Front)'), optional: false },
        { key: 'shop_interior', label: t('2. Interior (Stock Images)'), optional: true },
        { key: 'shop_godown', label: t('3. Godown Images'), optional: true }
      ].map((photo) => {
         ...
         return (
           <View key={photo.key} style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md, ...shadows.soft }}>
               ...
               <View key={index} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: colors.primarySoft, padding: 12, borderRadius: radius.md, marginBottom: 8 }}>
                 ...
                 <View style={{ flex: 1, marginLeft: 12 }}>
                   ...
                   {specificLocation && specificLocation.lat && specificLocation.lng ? (
                     ...
                     <Text style={{ color: colors.danger, fontSize: 11, fontWeight: '600', marginTop: 2 }}>⚠️ {t("GPS Failed")}</Text>
                   )}
                 </View>
             ...
             <Pressable onPress={() => handleUpload(photo.key, 'camera')} style={{ flexDirection: 'row', alignItems: 'center', padding: 14, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: radius.md, backgroundColor: '#F8FAFC' }}>
               ...
               <View style={{ flex: 1, marginLeft: 12 }}>
                 <Text style={{ color: colors.primary, fontWeight: '800', fontSize: 14 }}>{docsArray.length > 0 ? t("Capture Another Image") : t("Capture Image") + " " + t("GPS will be captured automatically")}</Text>
               </View>
             </Pressable>
           </View>
         );
      ...
      <Text style={{ fontWeight: '700', color: colors.textMuted, marginBottom: spacing.sm, marginTop: spacing.lg }}>{t("Selfie Verification")}</Text>
      <View style={{ backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md, ...shadows.soft }}>
        <Text style={{ fontWeight: '700', fontSize: 14, marginBottom: 8 }}>{t("Selfie with Contact Person")} *</Text>
        {watch('documents')?.['selfie_with_owner'] ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: colors.primarySoft, padding: 12, borderRadius: radius.md }}>
            <MaterialIcons name="check-circle" size={28} color={colors.primary} />
            <View style={{ flex: 1, marginLeft: 12 }}><Text style={{ color: colors.primary, fontWeight: '800', fontSize: 14 }}>{t("Selfie Captured")}</Text></View>
          ...
          <Pressable onPress={() => handleUpload('selfie_with_owner', 'camera')} style={{ flexDirection: 'row', alignItems: 'center', padding: 14, borderWidth: 1.5, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: radius.md, backgroundColor: '#F8FAFC' }}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={{ color: colors.primary, fontWeight: '800', fontSize: 14 }}>{t("Capture Selfie")}</Text>
              <Text style={{ color: colors.textMuted, fontSize: 11, marginTop: 2, fontWeight: '500' }}>{t("Take a clear photo with the contact person")}</Text>
            </View>
          </Pressable>
        )}
      </View>
      ...
      {watch('complianceChecklist')?.length > 0 && (
        <>
          <Text style={{ fontWeight: '700', color: colors.textMuted, marginBottom: spacing.sm, marginTop: spacing.lg }}>{t("Compliance Documents")}</Text>
      ...
      <Text style={{ fontWeight: '700', color: colors.textMuted, marginBottom: spacing.sm, marginTop: spacing.lg }}>{t("Additional Files")}</Text>
      <View style={{ marginBottom: spacing.md }}>
        <Text style={{ fontWeight: '700', fontSize: 14, marginBottom: 8, color: colors.text }}>{t("Farmer Customers List")} </Text>
